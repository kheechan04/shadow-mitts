...
**이런 점이 달라요**
- 🥊 **실제 복싱 번호 콤보** — "1-2-3"처럼 트레이너가 부르는 번호대로, 키보드·센서 없이 웹캠 하나로 친다
- 😆 **내 얼굴이 판정에 반응** — 사진 한 장으로 PERFECT엔 웃고 MISS엔 시무룩한 내 얼굴 (브라우저 안에서만 처리)
- 🎯 **약점 집중 연습** — 자주 틀리는 펀치를 기억해 다음 판에 더 자주 낸다
- 🔒 **설치 없이, 영상은 밖으로 안 나감** — 링크만 열면 되고, 앱처럼 설치(PWA)도 된다
...
플레이 테스트에서 나온 불편을 **데이터로 원인을 찾아** 고친 과정입니다. 자세한 근거는 [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md)의 결정 기록에 있습니다.
