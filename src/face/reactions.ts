// Reaction faces: one photo of the player → five expression images, entirely in this browser.
// Shared by the game (menu "리액션 얼굴") and the lab page (face.html).
//
// Privacy by construction — docs/FACE-PRIVACY.md lists what the on-screen notices promise:
// - photos and landmarks live only in canvases/variables; nothing is uploaded or stored
// - FaceLandmarker gives landmark positions only: no blendshapes/emotion analysis and no
...
export type { ExpressionName } from './warp';
...
export const EXPRESSION_ORDER: ExpressionName[] = ['perfect', 'good', 'normal', 'miss', 'critical'];

/** A face ready to be warped: a crop around it and its landmarks in crop pixels. */
...
/**
 * Find the face in a frame (un-mirrored camera pixels) and cut a crop around it with room for
 * hair, chin and the overlays. null when no face is found. The caller should wipe `src` after.
 */
export async function prepareFace(src: HTMLCanvasElement): Promise<PreparedFace | null> {
  const lmk = await ensureLandmarker();
  const face = lmk.detect(src).faceLandmarks[0];
  if (!face) return null;
  ...
  return { crop, lm: lm.map(([x, y]) => [(x - x0) * scale, (y - y0) * scale]) };
}

/** One expression, shown like a mirror (the way players see themselves). */
export function renderExpression(face: PreparedFace, expr: ExpressionName, strength = 1, overlays = true): HTMLCanvasElement {
  const to = warpLandmarks(face.lm, expr, strength);
  const warped = warpImage(face.crop, face.lm, to);
  if (overlays) overlay(warped.getContext('2d')!, expr, to);
  ...
  warped.width = warped.height = 1;
  return view;
}

export function renderAllExpressions(face: PreparedFace, strength = 1, overlays = true): Record<ExpressionName, HTMLCanvasElement> {
  const out = {} as Record<ExpressionName, HTMLCanvasElement>;
  for (const e of EXPRESSION_ORDER) out[e] = renderExpression(face, e, strength, overlays);
  return out;
}

/** Free the offscreen WebGL context used for warping (call when done making faces). */
...
/** Blank a canvas that held a photo (contents gone, memory released). */
export function wipe(c: HTMLCanvasElement | null | undefined): void {
  if (!c) return;
...
/** one small offscreen renderer, only while faces are being made — see disposeRenderer() */
